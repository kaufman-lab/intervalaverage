...
to do this job as fast and in as memory-efficient way as possible.
...
demonstrate the example of calculating long-term air pollution exposures
for individuals.
...
Note that each of these data sets is a `data.table` and the date columns
are `IDate`s. Because of a quirk of `data.table`, however, it is still
necessary to use `setDT` on each since they were loaded from the
package.
...
had modeled values, use the `required_percentage` argument. For example,
...
that can be useful in diagnosing why `NA`s are being returned (this is
...
sense, since more than one person often live in the same house! You'll
...
(for example, the full length of their address history). Or it could be
consistent for all people in the data set, such as a yearly average
...
### Full Address History Average

Again, the `y` argument of `intervalaverage` tells it what date range to
average for every group in the values data set. So we first need to create a 
`data.table` that contains the beginning and end of each person's full address
history. 

```{r}
address_coverage <- address_history[ , .(person_id, start = min(start_date), end = min(end_date)), by = 'person_id']
```
...
head(no2_overall[, .(person_id, start, end, no2)])
```

### Annual Averages

Finally, let's calculate annual (calendar-year) averages for 2000-2004. We'll need to build a `y` argument 
for the `intervalaverage` function that has a start date, end date for each year. However, we *also* need to repeat each
year once for every participant, because `y` must contain the same grouping variable(s) as `x`. 
