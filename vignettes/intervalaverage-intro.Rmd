...
First to make this more realistic, let's generate ~10 years of data at 2000 locations:

```{r}
n_locations <- 600
n_days_exposure_measurement <- 500
exposure_dataset3 <- rbindlist(
  lapply(1:n_locations, function(id) {
    data.table(
      location_id = id,
      start = seq(as.IDate("2000-01-01"), by = 7, length = n_days_exposure_measurement),
      end = seq(as.IDate("2000-01-07"), by = 7, length = n_days_exposure_measurement),
      pm25 = rnorm(n_days_exposure_measurement, mean = 15),
      no2 = rnorm(n_days_exposure_measurement, mean = 25)
    )
  })
)
...
```{r}
averaging_periods3 <- data.table(location_id=1:n_locations,
                                 start=sample(
                                   seq(as.IDate("2000-01-01"),as.IDate("2010-12-31"),by=1),
                                   n_locations
                                 )
)
...
earliest possible latest possible averaging period start date is Dec 31, 2010 but the exposures stop
in late 2009. The `required_percentage` argument could be set here to something less than the default of 100
...
sample_dates <- function(n){
  stopifnot(n%%2==0)
  dateseq <- seq(as.IDate("1995-01-01"),as.IDate("2010-01-01"),by=1)
