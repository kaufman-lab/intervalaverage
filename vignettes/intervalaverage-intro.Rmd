...
```{r}
exposure_dataset <- data.table(location_id=1L, 
                               start=seq(as.IDate("2000-01-01"),by=7L,length=4),
                               end=seq(as.IDate("2000-01-07"),by=7L,length=4),
...
```{r}
averaged_exposures <- intervalaverage(x=exposure_dataset,y=averaging_periods,                                      interval_vars=c("start","end"),value_vars=c("pm25","no2"))
averaged_exposures[, list(start,end,pm25,no2)]
```
The return value of the `intervalaverage` function is a `data.table`. Above are printed just the first four columns of that return object.  The return data.table always the exact intervals specified in y (and, as such,
...
```{r}
exposure_dataset2 <- rbindlist(lapply(1L:3L, function(z){
  data.table(location_id=z, 
             start=seq(as.IDate("2000-01-01"),by=7L,length=4),
             end=seq(as.IDate("2000-01-07"),by=7L,length=4),pm25=rnorm(4,mean=15),
             no2=rnorm(4,mean=25))} ))
exposure_dataset2
```
...
rbindlist(lapply(1L:3L, function(z)copy(averaging_periods)[,location_id:=z][])) 
```
...
Now, to take averages of `x` values over intervals in `y` within groups, all we have to do is use the same call as above to intervalaverage while specifying one more argument: `group_vars="location_id"`.
```{r}
...
```{r}
exposure_dataset_overlapping <- rbindlist(lapply(1L:3L, function(z){
  data.table(location_id=z, 
...
While overlapping periods in x are not allowed, there's nothing wrong with averaging to multiple overlapping
periods in y at the same time:
...
First to make this more realistic, let's generate ~5 years of data at several locations:

```{r}
n_locations <- 300L
n_weeks_exposure_measurement <- 300L
exposure_dataset3 <- rbindlist(
  lapply(1L:n_locations, function(id) {
    data.table(
      location_id = id,
      start = seq(as.IDate("2000-01-01"), by = 7L, length = n_weeks_exposure_measurement),
      end = seq(as.IDate("2000-01-07"), by = 7L, length = n_weeks_exposure_measurement),
      pm25 = rnorm(n_weeks_exposure_measurement, mean = 15),
      no2 = rnorm(n_weeks_exposure_measurement, mean = 25)
    )
  })
)
...
Now let's pick a different random end date for each location's averaging period. We'll define the end
date as 3 years after start that to define three-year intervals for every location.
```{r}
averaging_periods3 <- data.table(location_id=1L:n_locations,
                                 start=sample(
                                   seq(as.IDate("2000-01-01"),as.IDate("2005-12-31"),by=1L),
                                   n_locations
                                 )
)
...
earliest possible latest possible averaging period start date is Dec 31, 2005 but the exposures stop
in late 2005. The `required_percentage` argument could be set here to something less than the default of 100
...
overlapping periods and potentially make decisions about which address to use in each overlapping period).
...
`intervalintersect` is also an inner join beacuse rows from either table that are not joined are not 
...
starting with the unique set of locations extracted from `exposure_dataset3`, let's generate 150 participants and a random number of addresses each participant lived at
```{r}
n_ppt <- 150L
addr_history <- data.table(ppt_id=paste0("ppt",1L:n_ppt))
...
sample_dates <- function(n){
  stopifnot(n%%2==0)
  dateseq <- seq(as.IDate("1995-01-01"),as.IDate("2010-01-01"),by=1L)
...
addr_history_dates <- addr_history[,list(date=sample_dates(.N*2L)) ,by="ppt_id"] #for every address, ppt needs two dates: start and end
addr_history_dates_wide <- addr_history_dates[, list(start=date[(1L:.N)%%2L==1L],end=date[(1L:.N)%%2L==0L]),by="ppt_id"]
...
#addr_history[,any(end=="2030-01-01"),by="ppt_id"][, sum(V1)]
...
(agein, the `isolateoverlap` function would be useful for isolating sections of overlapping address intervals).
